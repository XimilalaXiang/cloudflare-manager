...
export default function Dashboard() {
  ...
  const stats = [
    { label: 'ACCOUNTS', value: accounts.length, color: 'bg-[#ff006e]' },
    { label: 'ACTIVE', value: accounts.filter((a) => a.status === 'active').length, color: 'bg-[#3a86ff]' },
    { label: 'INACTIVE', value: accounts.filter((a) => a.status !== 'active').length, color: 'bg-[#ffbe0b]' },
  ]
  ...
  return (
    <div className="py-8 md:py-12 px-4 md:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-end justify-between mb-8 md:mb-12">
          <div>
            <h1 className="font-black text-3xl md:text-5xl tracking-tight">
              DASH<span className="text-[#ff006e]">BOARD</span>
            </h1>
            <p className="font-mono text-sm md:text-base mt-2">
              Overview of your Cloudflare accounts
            </p>
          </div>
          <Link
            to="/accounts"
        ...
        <h2 className="font-black text-xl md:text-3xl mb-4 md:mb-6">ACCOUNTS</h2>
        ...
        {loading ? (
          ...
          <div className="border-2 md:border-4 border-black p-8 md:p-12 text-center">
            <p className="font-black text-xl md:text-2xl mb-4">NO ACCOUNTS YET</p>
            <p className="font-mono text-sm mb-6">Add your first Cloudflare account to get started.</p>
            <Link
              to="/accounts"
            ...
            {accounts.map((account) => (
              ...
              >
                ...
                <p className="font-mono text-xs text-black">
                  Token: {account.api_token_masked}
                </p>
              </div>
            ))}
          </div>
