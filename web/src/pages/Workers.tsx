    ...
    <div className="py-8 md:py-12 px-4 md:px-8">
      <div className="max-w-7xl mx-auto">
        <h1 className="font-black text-3xl md:text-5xl tracking-tight mb-2">
          WORK<span className="text-[#ffbe0b]">ERS</span>
        </h1>
        <p className="font-mono text-sm md:text-base mb-8">Manage Cloudflare Workers scripts</p>
        ...
        {loading ? (
          <div className="border-2 md:border-4 border-black p-8 text-center font-mono">Loading workers...</div>
        ) : workers.length === 0 ? (
          <div className="border-2 md:border-4 border-black p-8 md:p-12 text-center">
            <p className="font-black text-xl md:text-2xl mb-2">NO WORKERS</p>
            <p className="font-mono text-sm">
              {accounts.length === 0 ? 'Add an account first.' : 'No workers found for this account.'}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            {workers.map((w) => (
              ...
              >
                <h3 className="font-black text-lg md:text-xl mb-2">{w.id}</h3>
                <div className="font-mono text-xs space-y-1">
                  <p>Size: <span className="font-bold">{(w.size / 1024).toFixed(1)}KB</span></p>
                  <p>Created: {w.created_on?.slice(0, 10)}</p>
                  <p>Modified: {w.modified_on?.slice(0, 10)}</p>
                </div>
              </div>
            ))}
          </div>
